...
class TensorRTNet:
    ...
    def toTensorRT(self, filepath):
        ...
        del engine
        del runtime
    # -------------方式1：使用TensorRT构建Engine----------- END ---------- # 
    
    # -------------方式2：通过解析onnx，构建引擎（不需要搭建TensorRT网络结构)----#
    def ONNX_build_engine(self, onnx_file_path):
        ...
        with trt.Builder(G_LOGGER) as builder, \
            ...
            # 保存计划文件
            with open("../models/threeMnist.engine", "wb") as f:
                f.write(engine.serialize())
            return engine
    ...
    # -------方式3：通过加载engine，构建引擎（不需要搭建TensorRT网络结构）----#
    def loadEngine2TensorRT(self, filepath):
        ...
        del context
        del engine
    # -------方式3：通过加载engine，构建引擎（不需要搭建TensorRT网络结构）--END--#
